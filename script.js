...
function animateBubble(elem){
    ...
    function frame(){
        if(position >= (windowHeight + 150) && (document.
            querySelector('[data-number = "' + elem.dataset.number
            + '"]') !== null)){
...
function startGame(){
    ...
    let loop = setInterval(function(){
        ...
        if(!gameOver && noPop !== total){
            ...
            shadow.querySelector('.looser').style.display = 'block';
        }
        else {
            clearInterval(loop);
            shadow.style.display = 'flex';
...
document.querySelector('.restart').addEventListener('click', function(){
    ...
    shadow.querySelector('.looser').style.display = 'none';
    startGame();
});
